  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/portal/me' && request.method === 'GET') {
        ...
        const client_id = url.searchParams.get('client_id');
        if (!client_id) return bad('client_id required', cors);
        const [client, checkins, mealPlan, scans, sessions, selfWorkouts, tier, mealProfile] = await Promise.all([
        ...
        return ok({ client, checkins: checkins.results||[], mealPlan, scans: scans.results||[], sessions: sessions.results||[], selfWorkouts: selfWorkouts.results||[], tier, mealProfile: mealProfile||null }, cors);
      }
