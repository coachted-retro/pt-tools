...
const ALLOWED_TABLES = new Set([
  'partners','partner_taps','guest_shares','members','board_notes','notifications','huddle_messages','help_requests','coach_touchpoints','portal_messages','exercise_videos','scheduled_sessions',
...
Allergies: ${mp.allergies||'none'}. Medical conditions: ${mp.conditions||'none'}.`;
      ...
      if (url.pathname === '/coach/today-schedule' && request.method === 'GET') {
        ...
        const rows = await env.DB.prepare(
          `SELECT s.id, s.client_id, s.program_name, s.focus_notes, c.first_name, c.last_name
           ...
           WHERE c.coach = ? AND s.scheduled_date = ? AND s.status = 'scheduled'
      ...
      if (url.pathname === '/schedule/create' && request.method === 'POST') {
        ...
        for (let d = new Date(start); d <= end; d.setDate(d.getDate()+1)) {
          if (b.weekdays.includes(d.getDay())) {
            ...
            const ins = await env.DB.prepare(
              'INSERT INTO scheduled_sessions (client_id,scheduled_date,program_name,focus_notes,status,created_by) VALUES (?,?,?,?,?,?)'
            ).bind(b.client_id, dateStr, b.program_name, b.focus_notes||'', 'scheduled', b.coach_name||'').run();
      ...
      if (url.pathname === '/coach/client-detail' && request.method === 'GET') {
        ...
        const clientId = url.searchParams.get('client_id');
        if (!clientId) return bad('client_id required', cors);
        const [client, scans, sessions, selfWorkouts, photos, touchpoints, messages] = await Promise.all([
          ...
          env.DB.prepare('SELECT * FROM training_sessions WHERE client_id=? ORDER BY session_date DESC LIMIT 30').bind(clientId).all(),
        ...
        return ok({ client, scans: scans.results||[], sessions: sessions.results||[], selfWorkouts: selfWorkouts.results||[],
          photos: photos.results||[], touchpoints: touchpoints.results||[], messages: messages.results||[] }, cors);
      }
