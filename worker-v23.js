      ...
      if (url.pathname === '/schedule/create' && request.method === 'POST') {
        ...
        for (let d = new Date(start); d <= end; d.setDate(d.getDate()+1)) {
          if (b.weekdays.includes(d.getDay())) {
            ...
            const ins = await env.DB.prepare(
              'INSERT INTO scheduled_sessions (client_id,scheduled_date,program_name,focus_notes,status,created_by,assigned_coach) VALUES (?,?,?,?,?,?,?)'
            ).bind(b.client_id, dateStr, b.program_name, b.focus_notes||'', 'scheduled', b.coach_name||'', assignedCoach).run();
