...
const ALLOWED_TABLES = new Set([
  'partners','partner_taps','guest_shares','members','board_notes','notifications','huddle_messages',
...
Allergies: ${mp.allergies||'none'}. Medical conditions: ${mp.conditions||'none'}.`;
      ...
      if (url.pathname === '/sales/record' && request.method === 'POST') {
        ...
        const ins = await env.DB.prepare('INSERT INTO pt_sales (gym_id,member_id,client_name,package_name,sessions,amount,sold_by,sale_date) VALUES (?,?,?,?,?,?,?,?)')
          .bind(b.gym_id, b.member_id||null, b.client_name||'', b.package_name||'', b.sessions||null, b.amount, b.sold_by, saleDate).run();
        ...
        const winBody = '\uD83C\uDF89 ' + gymName + ' closed a ' + (b.package_name || 'PT package') + ', $' + amt + ', ' + b.sold_by;
      ...
      if (url.pathname === '/huddle/feed' && request.method === 'GET') {
        ...
        else rows = await env.DB.prepare('SELECT * FROM huddle_messages WHERE channel=? ORDER BY id DESC LIMIT 40').bind(channel).all();
...
async function getGymDetail(env, gymId, month){
  ...
  return { gym, month, goal: quota?.tcv_goal||0, actual: monthSales?.total||0, sale_count: monthSales?.n||0,
    reps: repRows, recent_sales: recent.results||[], trend };
}
